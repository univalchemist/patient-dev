import { AuthorizationAPI, CurrencyAPI } from 'api';
import React, {
  createContext,
  useMemo,
  useContext,
...
const AppContextProvider = ({ ...props }) => {
  ...
  const getMeData = async () => {
    try {
      ...
      const currencyResult = await CurrencyAPI.getAll();
      const usdConversionRate = +currencyResult.find((c: any) => c.code === 'USD').conversionRate;
      const eurConversionRate = +currencyResult.find((c: any) => c.code === 'EUR').conversionRate;
      ...
      setState({
        ...
        currencyRate: { usd: +usdConversionRate.toFixed(2), eur: +eurConversionRate.toFixed(2) }
      });
