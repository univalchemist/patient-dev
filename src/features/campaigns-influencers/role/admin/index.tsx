...
const CampaignInfluencersPage = ({ id }: { id: string }) => {
  ...
  const getCampaignInfluencers = useCallback(async () => {
    const { result, meta } =
      await PlatformProductAPI.getPlatformProductOrderCampaignInfluencers(+id, {
        limit: 10,
        skip: (page - 1) * 10,
      });
  ...
  // Table New Checkbox Modal controls
  ...
  const handleBulkInfluencerConfirmMatch = async () => {
    ...
    if (campaign) {
      CampaignAPI.confirmMatchingInfluencer(
        ...
        .then(() => {
          push('Successfully confirmed matching influencers in a campaign.', {
            variant: 'success',
          });
          setCheckedInfluencers([]);
          setCheckedPlatformInfluencers([]);
  ...
  const renderItem = ({ headItem, row }: TTableRenderItemObject) => {
    ...
    const allowedStatus = [2, 6, 7, 8, 9, 10, 12];

    if (
      headItem.reference === 'firstName' &&
      allowedStatus.includes(productInfluencer.status)
    ) {
