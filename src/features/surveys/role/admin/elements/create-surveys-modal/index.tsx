...
const CreateSurveysModal = ({
  ...
  const disabled = !state.surveyName || !state.client;
  ...
  const createSurvey = useCallback(async () => {
    try {
      const body = {
        ...
        diseaseAreaIds:
          state.diseaseArea && state.diseaseArea.length
            ? state.diseaseArea.map((x: any) => x.value)
            : [],
        struggleIds:
          state.struggles && state.struggles.length
            ? state.struggles.map((x: any) => x.value)
            : undefined,
        locationIds:
          state.location && state.location.length
            ? state.location.map((x: any) => x.value)
            : [],
        ethnicityIds:
          state.ethnicity && state.ethnicity.length
        ...
        interestIds:
          state.interests && state.interests.length
            ...
            : undefined,
        productIds:
          state.product && state.product.length
        ...
        languages:
          state.language && state.language.length
            ? state.language.map((x: any) => x.value)
            : [],
        clientUserId: state.client.value || undefined,
        stakeholderTypes:
          state.stakeholders && state.stakeholders.length
            ? state.stakeholders.map((x: any) => x.value)
            : undefined,
        symptomIds:
          state.symptoms && state.symptoms.length
        ...
        ChatIO.createChatRoom(
          {
            ...
            productOrderChatRoomMember: data.platformProductOrder.client.userId,
          },
          () => {}
        );
