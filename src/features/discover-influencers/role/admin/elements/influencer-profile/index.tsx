...
const InfluencerProfile = ({
  ...
  const updateInfluencer = async (body: any, id: any) => {
    if (!disabled) {
      ...
      const formData = {
        ...
        specialization: specialization?.value || null,
  ...
  useEffect(() => {
    if ([3, 4, 5].includes(influencer?.influencer.type)) {
      getSpecializations(influencer?.influencer.type);
      EnumsApi.getSpecializationTypes(influencer?.influencer.type).then(
        (result) => {
          ...
          const specialization = data.find(
            (item: TSelectFieldType) =>
              item.value === influencer?.influencer.specialization
          );
          setState((prevState: any) => ({ ...prevState, specialization }));
        }
      );
    }
  }, [influencer]);
  ...
  useEffect(() => {
    if (
      influencer &&
      genderOptions &&
      (![3, 4, 5].includes(influencer?.influencer.type) || specializations)
    ) {
      ...
      Promise.allSettled([postTypes, surveyTypes]).then((typesResults) => {
        ...
        if (
          ...
          setState((prevState: any) => {
            ...
            const username = influencer.influencer?.instagramUsername;
            ...
            return {
              ...
              affiliateCode: influencer.influencer.affiliateCode,
            };
          });
        }
      });
  ...
  const handleChangeExperience = (experience?: TSelectFieldType) => {
    setState({ ...state, experience, specialization: null });
    getSpecializations(experience?.value);
  };
      ...
      <Stack>
        <InfluencerProfileModalMain columns={2}>
          ...
          <Input
            ...
            onValue={handleChangeExperience}
            options={stakeholders}
          />
          {[3, 4, 5].includes(state.experience?.value) && (
            <Input
              type="select"
              ...
              onValue={(specialization) =>
                setState({ ...state, specialization })
              }
          ...
          <Input
            ...
            onValue={(affiliateCode) => setState({ ...state, affiliateCode })}
          />
        </InfluencerProfileModalMain>
      </Stack>
    </Modal>
