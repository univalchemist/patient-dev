...
const CampaignModal = ({
  ...
  const handleSubmissionLink = async () => {
    if (
      ...
      push(
        'Your Status needs to be Not Approved/To Be Submitted and Campaign needs to be in progress.',
        {
          variant: 'error',
        }
      );
  ...
  return (
    ...
    >
      ...
      >
        {/* 'Admin Chat', 'Chat' */}
        <Tabs tabs={tabs} value={tab} onValue={setTab} />
        {tab === 0 && (
            ...
            <GridCell columnSpan={1}>
              <ImageUploadMainContainer>
                {photos && photos.length
                  ? photos.map((item: TCampaignPhoto) => {
        ...
        {tab === 2 && (
          <AddCampaignsModalMain columns={2}>
            <Input
              type="text"
              label="Submission"
              placeholder="Please Paste Your Submission Link"
            ...
            <GridCell columnSpan={2}>
              {chatRoom ? (
                <Chat chatRoomId={chatRoom} isPingActive size={180} />
              ) : (
                <span />
              )}
            </GridCell>
