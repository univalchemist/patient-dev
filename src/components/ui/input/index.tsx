    ...
    <InputMain {...props}>
      ...
      {type === 'select' && (
        <InputSelect
          ...
          filterOptions={(filterOptions: any, params: any) => {
            const filteredOptions = filter(filterOptions, params);
            const { inputValue } = params;
            const isExisting = filterOptions.some(
              (filterOption: any) => inputValue === filterOption.label
            );

            const searchString = 'other';

            // Check if similar words exist in filterOption Labels
            const isExisting2 = filterOptions.some((filterOption: any) =>
              filterOption.label
                .toLowerCase()
            ...
            if (
              ...
              !isExisting &&
              !isExisting2 &&
              !searchString.includes(inputValue.toLowerCase())
            ) {
          ...
          renderOption={(optionProps, option: any) => (
            <MenuItem key={`${label}'_'${option.value}`} {...optionProps}>
              {option.label}
            </MenuItem>
          )}
          renderInput={({
