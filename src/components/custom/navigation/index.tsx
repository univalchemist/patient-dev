...
const Navigation = ({ ...props }: TNavigationProps) => {
  ...
  const getBalance = async () => {
    try {
      ...
      if (response.availableAmounts) {
        ...
        setFormattedBalance(() =>
          handleCurrencyCalculation(response.availableAmounts, currencyRate, 'CHF')
        );
      }
    } catch (error) {
  ...
  const handleCurrencyChangeVal = useCallback(
    (currencyVal: 'EUR' | 'USD' | 'CHF') => {
      ...
      setOpenR(!openR);
      setFormattedBalance(() =>
        handleCurrencyCalculation(balance, currencyRate, currencyVal)
      );
    },
    [balance, openR]
  );
  ...
  const handleCurrencyAndBalanceChange = (
    ...
    setFormattedBalance(() => handleCurrencyCalculation(balance, currencyRate, currencyVal));
  };
